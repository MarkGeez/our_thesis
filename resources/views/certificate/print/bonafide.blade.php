...
<head>
  ...
  <title>Certification - Bonafide</title>
  <style>
    ...
    .header-row { display: flex; justify-content: space-between; align-items: center; padding: 10px 20px; border-bottom: 10px solid #180b5f; box-shadow: 0 10px 0 yellow; }
    .logo-small{width:80px;height:auto}
    .content{display:flex;padding:15px}
    .left { width: 30%; border-right: 6px solid yellow; border-bottom: 6px solid yellow; padding-right: 1px; position: relative; box-shadow: 7px 0 0 red; min-height: 800px; }
...
at Biak na Bato St. Corner Almeda Tondo, Manila

        @else
          <span class="fill-line">{{ $name }}</span> of legal age is a bonafide resident of BARANGAY 249 ZONE 23 with postal address at Biak na Bato St. Corner Almeda Tondo, Manila
...
@endforeach
      ...
      <div class="issued">Given this
        @if($editable)
          <input type="text" class="fill-line" name="issued_day" value="{{ $issued->format('j') }}" style="width:40px"> day of
          <input type="text" class="fill-line" name="issued_month" value="{{ $issued->format('F') }}" style="width:100px">
          <input type="text" class="fill-line" name="issued_year" value="{{ $issued->format('Y') }}" style="width:50px">
        @else
          {{ $issued->format('j') }} day of {{ $issued->format('F') }} {{ $issued->format('Y') }}
        @endif
        at Barangay 249 hall Zone 23 District II, Manila</div>
